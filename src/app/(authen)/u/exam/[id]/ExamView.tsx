...
function ExamView({ exam, setResult }: IExamView) {
  ...
  useEffect(() => {
    try {
      ...
      const answers = sessionStorage.getItem("ans   wers");
  ...
  useEffect(() => {
    if (!isSubmitted && timeRemaining > 0) {
      sessionStorage.setItem("time", (timeRemaining - 1).toString());
          ...
          <CardContent className="p-4">
            <div className="flex flex-col gap-3">
              <h2 className="text-xl font-semibold">
                {exam.title} (Đề {exam.examCode.split("_").pop()})
              </h2>
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-4 flx-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <User className="h-4 w-4 text-gray-500" />
                    <span className="text-ellipsis overflow-hidden whitespace-nowrap">{account?.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <CardHeader>
            <div className="space-y-2">
              <div className="flex justify-between text-gray-600 text-base font-medium">
                <span>
                  Câu {currentQuestionIndex + 1}/{exam.totalQuestions}
                </span>
          ...
          <CardContent className="space-y-6">
            <div>
              ...
              <RadioGroup.Root
                ...
                onValueChange={(e) => handleAnswerSelect(currentQuestion.STT!, e)}
              >
                <div className="space-y-3">
                  {["A", "B", "C", "D"].map((option, index) => (
                    <div
                      onClick={() => handleAnswerSelect(currentQuestion.STT!, option)}
                      key={index}
                      className={clsx(
                        "flex items-center space-x-2 gap-2 border rounded-lg p-4 hover:bg-gray-50 cursor-pointer",
                        option === answers[currentQuestion.STT!] && "bg-green-50 border-green-500!",
                      )}
                    >
            ...
            <div className="flex justify-between">
              ...
              {currentQuestionIndex === exam.totalQuestions - 1 ? (
                Object.keys(answers).length === exam.totalQuestions ? (
                  <Button loading={isSubmitted} disabled={isSubmitted} onClick={handleSubmit} size="sm">
                    Nộp bài
                  </Button>
                ) : (
                  <ButtonSubmitExam isSubmitted={isSubmitted} handleSubmit={handleSubmit} />
                )
              ) : (
                <Button variant="basic" size="sm" onClick={() => setCurrentQuestionIndex((prev) => prev + 1)}>
                  Câu tiếp theo
                </Button>
              )}
            </div>
              ...
              {exam.questions.map((q, index) => (
                <button
                  ...
                  className={clsx(
                    ...
                    currentQuestionIndex === index && "bg-yellow-100 border-yellow-600",
                    !!answers[index + 1] && "bg-green-100! border-green-600!",
                  )}
                >
                  {index + 1}
                </button>
