...
export async function POST(req: NextRequest) {
  try {
    const { examCode, answers } = await req.json();
    ...
    const details = originalExam.map((q) => {
      ...
      const userAns = answers.find((a: any) => a.questionNumber === qNum);
      const isCorrect = userAns?.selected === correctAnswer;

      if (isCorrect) correctCount++;

      return {
        questionNumber: qNum,
        userSelected: userAns?.selected || null,
