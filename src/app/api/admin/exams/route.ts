...
export async function GET() {
    try {
        ...
        return NextResponse.json(
            { error: "Không thể lấy danh sách bộ đề" },
            { status: 500 },
        );
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (
            ...
            return NextResponse.json(
                {
                    error: "Số lượng đề, số câu hỏi, số câu trả lời đạt và thời gian phải lớn hơn 0",
                },
                { status: 400 },
            );
        }
        ...
        const questions: any[] = XLSX.utils.sheet_to_json(
            bankWorkbook.Sheets[bankWorkbook.SheetNames[0]],
        );

        if (questions.length < numQuestions) {
            return NextResponse.json(
                { error: "Ngân hàng không đủ câu hỏi" },
                { status: 400 },
            );
        }
        ...
        const excelName =
            `${title}_${numExams}_DE_${numQuestions}_CAU_HOI_${Date.now()}.xlsx`.replace(
                /\s+/g,
                "_",
            );
        ...
        const newConfig = {
            ...
            createdAt:
                existingIndex !== -1
                    ? configs[existingIndex].createdAt
        ...
        if (existingIndex !== -1) {
            ...
            const oldFilePath = path.join(
                EXAM_DIR,
                configs[existingIndex].excelFile,
            );
